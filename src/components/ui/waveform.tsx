...
export const AudioScrubber = ({
  ...
  return (
    ...
    >
      ...
      <div
        className="bg-slate-900/20 pointer-events-none absolute inset-y-0 left-0 dark:bg-slate-50/20"
        style={{ width: `${localProgress * 100}%` }}
      />

      <div
        className="bg-slate-900 pointer-events-none absolute top-0 bottom-0 w-0.5 dark:bg-slate-50"
        style={{ left: `${localProgress * 100}%` }}
      />

      {showHandle && (
        <div
          className="border-white bg-slate-900 pointer-events-none absolute top-1/2 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 shadow-lg transition-transform hover:scale-110 dark:border-slate-950 dark:bg-slate-50"
          style={{ left: `${localProgress * 100}%` }}
        />
      )}
    </div>
