...
import { useScribe, type AudioFormat, type CommitStrategy } from "@/hooks/use-scribe";
...
function useSpeechInput() {
  ...
  if (!context) {
    throw new Error("SpeechInput compound components must be used within a");
  }
  return context;
}
  ...
  return (
    <SpeechInputContext.Provider value={contextValue}>
      <div
        ref={ref}
        className={cn(
          "relative inline-flex items-center overflow-hidden rounded-lg border border-slate-200 border-transparent transition-all duration-200 dark:border-slate-800",
          scribe.isConnected
            ? "bg-white dark:bg-slate-100 border-slate-200 shadow-sm dark:bg-slate-950 dark:dark:bg-slate-800 dark:border-slate-800"
            : "",
          className,
        )}
      >
  ...
  function SpeechInputRecordButton(
    ...
    return (
      ...
      >
        <div
          className={cn(
            "bg-slate-900 absolute h-4 w-4 rounded-full transition-all duration-200 dark:bg-slate-50",
            speechInput.isConnecting ? "scale-90 opacity-100" : "scale-[60%] opacity-0",
          )}
        />
        <SquareIcon
          className={cn(
            "text-red-500 absolute h-4 w-4 fill-current transition-all duration-200 dark:text-red-900",
      ...
      <div
        ...
        className={cn(
          ...
          showPlaceholder
            ...
            : "text-slate-500 [mask-image:linear-gradient(to_right,transparent,black_16px,black_calc(100%-16px),transparent)] dark:text-slate-400",
