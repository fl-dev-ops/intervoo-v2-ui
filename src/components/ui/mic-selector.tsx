...
export function MicSelector({
  ...
  const devices = providedDevices ?? discoveredDevices;
  const loading = providedLoading ?? discoveredLoading;
  const error = providedError ?? discoveredError;
  ...
  const shouldShowFooter = devices.length > 0 && (showMuteControl || showWaveformPreview);
    ...
    <DropdownMenu onOpenChange={handleDropdownOpenChange}>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          size="sm"
          className={cn(
            "hover:bg-slate-100 flex w-48 cursor-pointer items-center gap-1.5 dark:hover:bg-slate-800",
            className,
          )}
        ...
        {shouldShowFooter && (
          <>
            ...
            <div className="flex items-center gap-2 p-2">
              ...
              {showWaveformPreview ? (
                <div className="bg-slate-100 ml-auto w-16 overflow-hidden rounded-md p-1.5 dark:bg-slate-800">
                  <LiveWaveform
