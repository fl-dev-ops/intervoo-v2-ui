...
- Use the transcript as the primary source for evaluation.
- Return ONLY valid JSON matching the provided schema.
- Do not include markdown, commentary, or code fences.
- Keep strengths and improvement_areas concise and actionable (1-2 sentences each).
- Keep education_summary factual and short (1-2 sentences).
- Keep aspiration_statement and reality_statement concise and grounded in what the learner said.
- strengths MUST contain at least 1 item and at most 5.
- improvement_areas MUST contain at least 1 item and at most 5.
...
- Include concise reasons for every criterion label (thinking_reasoning, confidence_reasoning, and each language_reasoning dimension).
- Indian accent, Indian English vocabulary, and code-mixing are never penalised.

Diagnostic conversation goal:
- Understand the learner's background and goals
- Assess their thinking clarity, communication confidence, and English language proficiency
- Provide actionable feedback for interview preparation

Scoring framework:
- thinking_level: TF1 to TF5
- confidence_level: VCP1 to VCP4
...
Framework note: Holistic listener impression across delivery signals. Not a checklist. Score by overall pattern.
Sub-dimensions: Volume, Pace, Pause, Latency, Composure
...
- Composure: Audible nervousness throughout; voice tremor or filler dominance.
...
- Composure: Nervousness visible on harder questions; recovers partially.
...
- Composure: Generally composed; minor wobble on unexpected questions.
...
- Composure: Fully composed; handles unexpected questions with poise.
...
L1 (Pre-A1 / A1)
- Fluency: Speech rate consistently below 80 WPM. Mid-sentence pauses regularly exceeding 3 seconds. Majority of turns are single words or fragments. No self-repair - candidate simply stops. Filler words absent - output too minimal to contain them.
...
export function buildDiagnosticRubric(context: DiagnosticPromptContext) {
  const roundLabel = context.roundId
    ...
    : "Round: General diagnostic";
  const jobLabel = context.jobTitle
    ? `Target job: ${context.jobTitle} (${context.jobBand ?? "unknown band"})`
    : "";

  return `${DIAGNOSTICS_RUBRIC}

${roundLabel}
${jobLabel}
Learner name: ${context.participantName}

Transcript:
${context.transcriptPromptText}`;
}
