...
const englishLevelTextByValue = {
  native: "Fully comfortable speaking English.",
...
const englishLevels = (["native", "advanced", "intermediate", "beginner"] as const).map(
  (value) => ({
    value,
    text: englishLevelTextByValue[value],
  }),
);
...
export function PreScreeningPage() {
  ...
  const startLiveKitSession = useCallback(async () => {
  ...
  }, [connection, isConnecting, setStep, startLiveKitSession, step]);
            ...
            <div className="mt-4 space-y-2">
              {englishLevels.map((option) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <div className="text-sm font-semibold text-slate-100">{option.text}</div>
                    </div>
                    <div
                      className={cn(
