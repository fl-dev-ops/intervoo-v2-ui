import { GoogleGenAI } from "@google/genai";
import { FatalError, getStepMetadata, RetryableError } from "workflow";
import { z } from "zod";
import { prisma } from "@/lib/db";
import {
  deleteGeminiFiles,
  FileState,
  type GeminiUploadInputFile,
  getGeminiApiKey,
  getRecordingMimeType,
  type UploadedGeminiFile,
  uploadedGeminiFileStatus,
  uploadGeminiFile,
} from "@/lib/gemini-client";
import { updateUserStage } from "@/lib/progress";
import {
  type PreDiagnosticReport,
  PreDiagnosticReportSchema,
} from "@/lib/report-generation/prediagnostic";
import { buildPreDiagnosticRubric } from "@/lib/report-generation/rubrics";
import { buildReportTranscriptPromptText } from "@/lib/report-generation/transcript";
import { createUniquePublicReportToken } from "@/lib/report-share";
import { buildPublicReportUrl } from "@/lib/share-token";
import { sendWhatsAppReportLink } from "@/lib/twilio";

type PreDiagnosticReportGenerationResult = PreDiagnosticReport;

type WorkflowResult = {
  sessionId: string;
  shareToken: string;
  status: "READY";
};
type ReadyGeminiFile = UploadedGeminiFile & { uri: string };

const TRANSIENT_STATUSES = new Set([408, 429, 500, 502, 503, 504]);
const FILE_ACTIVE_POLL_INTERVAL_MS = 1500;
const FILE_ACTIVE_POLL_TIMEOUT_MS = 120_000;

export async function generatePrediagnosticSessionReportWorkflow(
  sessionId: string,
  baseUrl: string,
): Promise<WorkflowResult> {
  "use workflow";

  let uploadedFiles: ReadyGeminiFile[] = [];

  try {
    await preparePrediagnosticReportStep(sessionId);
    uploadedFiles = await preparePrediagnosticGeminiFilesStep(sessionId);
    const generated = await generatePrediagnosticReportStep(
      sessionId,
      uploadedFiles,
    );
    const persisted = await persistPrediagnosticReportStep(
      sessionId,
      generated,
    );
    await sendPrediagnosticReportLinkStep(
      sessionId,
      baseUrl,
      persisted.shareToken,
    );
    await cleanupPrediagnosticGeminiFilesStep(uploadedFiles);
    return persisted;
  } catch (error) {
    await cleanupPrediagnosticGeminiFilesStep(uploadedFiles);
    await markPrediagnosticReportFailedStep(sessionId, getErrorMessage(error));
    throw error;
  }
}

async function preparePrediagnosticReportStep(sessionId: string) {
  "use step";

  const session = await prisma.interviewSession.findUnique({
    where: { id: sessionId },
    include: { report: true },
  });

  if (!session) {
    throw new FatalError("Session not found");
  }

  if (session.type !== "PREDIAGNOSTIC") {
    throw new FatalError(`Unsupported session type: ${session.type}`);
  }

  await prisma.report.upsert({
    where: { sessionId: session.id },
    create: {
      sessionId: session.id,
      status: "PROCESSING",
      startedAt: new Date(),
    },
    update: {
      status: "PROCESSING",
      errorMessage: null,
      startedAt: new Date(),
      completedAt: null,
      failedAt: null,
    },
  });

  console.info("[diagnostics] prediagnostic report workflow prepared", {
    sessionId,
  });
}

async function preparePrediagnosticGeminiFilesStep(
  sessionId: string,
): Promise<ReadyGeminiFile[]> {
  "use step";

  const session = await prisma.interviewSession.findUnique({
    where: { id: sessionId },
    select: { audioUrl: true, transcript: true, transcriptUrl: true },
  });

  if (!session) {
    throw new FatalError("Session not found");
  }
  if (!session.audioUrl) {
    throw new FatalError("No audio recording is available for this session");
  }
  if (!session.transcriptUrl && !session.transcript) {
    throw new FatalError("No transcript is available for this session yet");
  }

  const transcriptFile: GeminiUploadInputFile = session.transcriptUrl
    ? {
        kind: "url",
        presignedUrl: session.transcriptUrl,
        mimeType: "text/plain",
        displayName: `prediagnostic-${sessionId}-transcript`,
      }
    : {
        kind: "content",
        content: JSON.stringify(session.transcript),
        mimeType: "text/plain",
        displayName: `prediagnostic-${sessionId}-transcript`,
      };

  const uploadedFiles: UploadedGeminiFile[] = [];

  try {
    uploadedFiles.push(
      await uploadGeminiFile({
        file: {
          kind: "url",
          presignedUrl: session.audioUrl,
          mimeType: getRecordingMimeType(session.audioUrl),
          displayName: `prediagnostic-${sessionId}`,
        },
        requestId: `prediagnostic-${sessionId}`,
      }),
    );
    uploadedFiles.push(
      await uploadGeminiFile({
        file: transcriptFile,
        requestId: `prediagnostic-${sessionId}`,
      }),
    );
  } catch (error) {
    await deleteGeminiFiles(uploadedFiles);
    throw error;
  }

  const readyFiles = await waitForGeminiFiles({
    files: uploadedFiles,
    requestId: `prediagnostic-${sessionId}`,
  });

  console.info("[diagnostics] prediagnostic Gemini files ready", {
    fileCount: readyFiles.length,
    sessionId,
  });

  return readyFiles;
}

async function generatePrediagnosticReportStep(
  sessionId: string,
  uploadedFiles: ReadyGeminiFile[],
): Promise<PreDiagnosticReportGenerationResult> {
  "use step";

  try {
    const session = await prisma.interviewSession.findUnique({
      where: { id: sessionId },
      include: { user: { include: { profile: true } } },
    });

    if (!session) {
      throw new FatalError("Session not found");
    }

    if (session.type !== "PREDIAGNOSTIC") {
      throw new FatalError(
        `Session type is not PREDIAGNOSTIC: ${session.type}`,
      );
    }

    if (!session.audioUrl) {
      throw new FatalError("No audio recording is available for this session");
    }

    const transcriptPromptText = buildReportTranscriptPromptText(
      session.transcript,
    );

    if (!transcriptPromptText) {
      throw new FatalError("No transcript is available for this session yet");
    }

    const profile = session.user.profile;
    const prompt = `${buildPreDiagnosticRubric({
      name: profile?.preferredName || session.user.name,
      college: profile?.institution,
      degree: profile?.degree,
      stream: profile?.stream,
      year: profile?.yearOfStudy,
    })}

Conversation transcript (ordered, includes student and agent):
${transcriptPromptText}`;

    const modelId =
      process.env.PREDIAGNOSTIC_REPORT_MODEL_ID ??
      process.env.DIAGNOSTIC_REPORT_MODEL_ID ??
      "gemini-2.5-flash";
    const ai = new GoogleGenAI({ apiKey: getGeminiApiKey() });

    console.info("[diagnostics] generating prediagnostic report", {
      fileCount: uploadedFiles.length,
      modelId,
      sessionId,
    });

    const result = await ai.models.generateContent({
      model: modelId,
      contents: [
        {
          role: "user",
          parts: [
            { text: prompt },
            ...uploadedFiles.map((file) => ({
              fileData: {
                fileUri: file.uri,
                mimeType: file.mimeType,
              },
            })),
          ],
        },
      ],
      config: {
        temperature: 0,
        responseMimeType: "application/json",
        responseJsonSchema: sanitizeJsonSchema(
          z.toJSONSchema(PreDiagnosticReportSchema),
        ),
      },
    });

    if (!result.text) {
      throw new Error("Gemini returned an empty response");
    }

    return PreDiagnosticReportSchema.parse(JSON.parse(result.text));
  } catch (error) {
    const status = getErrorStatus(error);
    const message = getErrorMessage(error);

    if (
      (status && TRANSIENT_STATUSES.has(status)) ||
      isTransientProviderError(message)
    ) {
      const metadata = getStepMetadata();
      console.warn("[diagnostics] retrying prediagnostic report generation", {
        attempt: metadata.attempt,
        error: message,
        sessionId,
        status,
      });
      throw new RetryableError(message, {
        retryAfter: Math.min(metadata.attempt ** 2 * 5_000, 60_000),
      });
    }

    if (isFatalPrediagnosticReportError(message)) {
      throw new FatalError(message);
    }

    throw error;
  }
}

generatePrediagnosticReportStep.maxRetries = 3;

async function cleanupPrediagnosticGeminiFilesStep(
  files: UploadedGeminiFile[],
) {
  "use step";

  if (!files.length) {
    return;
  }

  await deleteGeminiFiles(files);
}

async function waitForGeminiFiles(input: {
  files: UploadedGeminiFile[];
  requestId: string;
}): Promise<ReadyGeminiFile[]> {
  const readyFiles: ReadyGeminiFile[] = [];

  for (const file of input.files) {
    const deadline = Date.now() + FILE_ACTIVE_POLL_TIMEOUT_MS;
    let status = await uploadedGeminiFileStatus(file);

    while (status.state === FileState.PROCESSING) {
      if (Date.now() > deadline) {
        throw new Error("Gemini file did not become ACTIVE before timeout");
      }

      await new Promise((resolve) =>
        setTimeout(resolve, FILE_ACTIVE_POLL_INTERVAL_MS),
      );
      status = await uploadedGeminiFileStatus(status);
    }

    if (status.state === FileState.FAILED) {
      throw new Error(
        `Gemini file processing failed: ${status.errorMessage ?? "unknown error"}`,
      );
    }
    if (!status.uri) {
      throw new Error("Gemini upload returned no file URI");
    }

    console.info("[diagnostics] Gemini file active", {
      fileName: status.name,
      mimeType: status.mimeType,
      requestId: input.requestId,
    });

    readyFiles.push({
      name: status.name,
      uri: status.uri,
      mimeType: status.mimeType,
    });
  }

  return readyFiles;
}

function sanitizeJsonSchema(schema: unknown): unknown {
  if (Array.isArray(schema)) {
    return schema.map((item) => sanitizeJsonSchema(item));
  }
  if (!schema || typeof schema !== "object") {
    return schema;
  }

  const out: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(
    schema as Record<string, unknown>,
  )) {
    if (key === "$schema") continue;
    out[key] = sanitizeJsonSchema(value);
  }
  return out;
}

async function persistPrediagnosticReportStep(
  sessionId: string,
  reportJson: PreDiagnosticReportGenerationResult,
): Promise<WorkflowResult> {
  "use step";

  const existingReport = await prisma.report.findUnique({
    where: { sessionId },
    select: { shareToken: true },
  });

  const session = await prisma.interviewSession.findUnique({
    where: { id: sessionId },
    select: { userId: true },
  });

  if (!session) {
    throw new FatalError("Session not found");
  }

  const shareToken =
    existingReport?.shareToken ?? (await createUniquePublicReportToken());

  await prisma.report.upsert({
    where: { sessionId },
    create: {
      sessionId,
      reportJson: reportJson as object,
      status: "READY",
      completedAt: new Date(),
      shareToken,
    },
    update: {
      reportJson: reportJson as object,
      status: "READY",
      completedAt: new Date(),
      failedAt: null,
      errorMessage: null,
      shareToken,
    },
  });

  await prisma.interviewSession.update({
    where: { id: sessionId },
    data: { status: "COMPLETED" },
  });

  await updateUserStage(session.userId, "DIAGNOSTICS");
  await prisma.userProgress.updateMany({
    where: { userId: session.userId },
    data: { prediagnosticsCompletedAt: new Date() },
  });

  console.info("[diagnostics] prediagnostic report workflow ready", {
    sessionId,
    shareToken,
    userId: session.userId,
  });

  return { sessionId, shareToken, status: "READY" };
}

async function sendPrediagnosticReportLinkStep(
  sessionId: string,
  baseUrl: string,
  shareToken: string | null,
) {
  "use step";

  if (!shareToken) {
    return;
  }

  try {
    const session = await prisma.interviewSession.findUnique({
      where: { id: sessionId },
      select: { userId: true },
    });
    if (!session) {
      return;
    }

    const user = await prisma.user.findUnique({
      where: { id: session.userId },
      select: { name: true, phoneNumber: true },
    });

    if (!user?.phoneNumber) {
      return;
    }

    await sendWhatsAppReportLink(
      user.phoneNumber,
      user.name || "Learner",
      buildPublicReportUrl(baseUrl, shareToken, "pre"),
      "prediagnostic",
    );
  } catch (error) {
    console.error("Failed to send WhatsApp prediagnostic report link:", error);
  }
}

async function markPrediagnosticReportFailedStep(
  sessionId: string,
  errorMessage: string,
) {
  "use step";

  await prisma.report.updateMany({
    where: { sessionId, status: { not: "READY" } },
    data: {
      status: "FAILED",
      errorMessage,
      failedAt: new Date(),
    },
  });

  console.info("[diagnostics] prediagnostic report workflow failed", {
    error: errorMessage,
    sessionId,
  });
}

function getErrorStatus(error: unknown): number | null {
  if (error && typeof error === "object" && "status" in error) {
    const status = (error as { status?: unknown }).status;
    if (typeof status === "number") {
      return status;
    }
  }

  if (error && typeof error === "object" && "statusCode" in error) {
    const status = (error as { statusCode?: unknown }).statusCode;
    if (typeof status === "number") {
      return status;
    }
  }

  const message = getErrorMessage(error);
  try {
    const parsed = JSON.parse(message) as {
      error?: { code?: unknown; status?: unknown };
    };
    const code = parsed.error?.code;
    return typeof code === "number" ? code : null;
  } catch {
    return null;
  }
}

function getErrorMessage(error: unknown): string {
  if (error && typeof error === "object" && "cause" in error) {
    const cause = (error as { cause?: unknown }).cause;
    if (cause instanceof Error) {
      return cause.message;
    }
    if (cause && typeof cause === "object" && "message" in cause) {
      const message = (cause as { message?: unknown }).message;
      if (typeof message === "string" && message.trim()) {
        return message;
      }
    }
  }

  if (error instanceof Error) {
    return error.message;
  }

  if (error && typeof error === "object" && "message" in error) {
    const message = (error as { message?: unknown }).message;
    if (typeof message === "string" && message.trim()) {
      return message;
    }
  }

  if (typeof error === "string") {
    return error;
  }
  return "Pre-diagnostic report generation failed";
}

function isTransientProviderError(message: string) {
  const normalized = message.toLowerCase();
  return [
    "rate limit",
    "rate_limit",
    "timeout",
    "timed out",
    "service unavailable",
    "temporarily unavailable",
    "connection reset",
    "econnreset",
    "socket hang up",
    "bad gateway",
    "gateway timeout",
    "too many requests",
  ].some((text) => normalized.includes(text));
}

function isFatalPrediagnosticReportError(message: string) {
  return [
    "Session not found",
    "Session type is not PREDIAGNOSTIC",
    "No audio recording is available for this session",
    "No transcript is available for this session yet",
  ].some((fatalMessage) => message.includes(fatalMessage));
}
